...
- **Aviso**: a colour-coded warning card in the four AEMET levels (verde, amarillo, naranja, rojo) with the phenomenon. Worldwide it shows a threshold advisory (storm, heavy rain, snow, wind, heat, cold, fog) derived from the numbers; in Spain, with an AEMET key, the official AEMET aviso (its CAP warning for your area) overrides it. Shown only when a warning is active.
...
- **Sun and Moon**: by day, a daylight arc with the sun riding sunrise to sunset and the times at the ends, the golden hour marked at each end of the arc with a readout of when the next one falls; after dark, the moon drawn at its real phase with the phase name and illumination, and a Blue hour heading during the civil-twilight window.
